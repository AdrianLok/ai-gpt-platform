...
import { BottomStatusBar } from "../panels/BottomStatusBar";
import { CanvasGrid } from "./CanvasGrid";
import { CanvasToolbar } from "./CanvasToolbar";
import { CanvasViewport } from "./CanvasViewport";
import { DraftImageNode, ImageNode } from "./ImageNode";
import { PromptBar } from "../generation/PromptPanel";
...
type CanvasAreaProps = {
  ...
  onDuplicateGeneratedNode: (id: string) => void;
  onRenameGeneratedNode: (id: string, title: string) => void;
  onToggleGeneratedNodeCollapse: (id: string) => void;
...
export const CanvasArea = memo(function CanvasArea({
  ...
  onDuplicateGeneratedNode,
  onRenameGeneratedNode,
  onToggleGeneratedNodeCollapse,
  onDownloadImage,
  onOpenPreview,
  onCopyPrompt,
  ...
  const fitView = () => {
    ...
    onUpdateZoom(defaultCanvasZoom);
  };

  useCanvasShortcuts({
    selectedNodeId,
    onDeleteSelected: () => {
      if (selectedNodeId === "draft-image") {
        onDeleteDraftNode();
        return;
      }
      if (selectedNodeId) onDeleteGeneratedNode(selectedNodeId);
    },
    onDuplicateSelected: () => {
      if (selectedNodeId && selectedNodeId !== "draft-image") {
        onDuplicateGeneratedNode(selectedNodeId);
      }
    },
    onFitView: fitView,
  });
  ...
  return (
    <>
      <section
        ...
        onDoubleClick={(event) => {
          if (viewport.isSpacePressed) return;
        ...
        onPointerDown={(event) => {
          ...
          if (viewport.beginPan(event.clientX, event.clientY)) {
      ...
      >
        <CanvasGrid offset={viewport.offset} zoom={canvasZoom} />
        <CanvasToolbar onFitView={fitView} />

        <CanvasViewport offset={viewport.offset}>
          <AddNodeMenu
            menu={addNodeMenu}
          ...
          {referenceImages.map((item) => (
            <ReferenceImageNode
              ...
              dragging={activeDragNodeId === item.id}
          ...
          {generatedNodes.map((node) => (
            <ImageNode
              ...
              dragging={activeDragNodeId === node.id}
              ...
              onDuplicate={onDuplicateGeneratedNode}
              onRename={onRenameGeneratedNode}
              onToggleCollapse={onToggleGeneratedNodeCollapse}
          ...
          {!nodeCreated ? (
            ...
            <>
              <DraftImageNode
                ...
                dragging={activeDragNodeId === "draft-image"}
...
export const Canvas = CanvasArea;
  ...
  return (
    <article
      ...
      style={{
        ...
        transform: `translate3d(-50%, -50%, 0) scale(${canvasRenderScale * (dragging ? 1.025 : 1)})`,
        ...
        boxShadow: dragging
          ? "0 32px 110px rgba(255,255,255,0.18)"
...
function ZoomControls({ canvasZoom, onUpdateZoom, onFitView }: ZoomControlsProps) {
  return (
    <div
      style={{
        position: "absolute",
