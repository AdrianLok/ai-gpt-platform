...
export const ImageNode = memo(function ImageNode({
  ...
  const nodeTitle = node.title || node.prompt?.slice(0, 16) || "图片生成";
  ...
  return (
    <article
      ...
      style={{
        ...
        touchAction: "none",
        userSelect: "none",
        zIndex: selected || dragging ? 8 : 5,
        transition: dragging ? "none" : "transform 140ms ease, filter 140ms ease",
      }}
    >
      <div
        style={{
          ...
          {menuOpen ? (
            ...
            >
              {[
                ...
                { label: "复制节点", action: () => onDuplicate(node.id) },
                {
                  label: "重命名",
                  action: () => {
                    const title = window.prompt("节点名称", nodeTitle);
                    if (title !== null) onRename(node.id, title.trim() || nodeTitle);
                  },
                },
                { label: node.collapsed ? "展开节点" : "折叠节点", action: () => onToggleCollapse(node.id) },
      ...
      <div
        style={{
          ...
          border: selected
            ...
            : dragging
              ? "2px solid rgba(134,239,172,0.78)"
          ...
          boxShadow: selected
            ...
            : dragging
              ? "0 34px 120px rgba(134,239,172,0.16), 0 0 0 1px rgba(134,239,172,0.18)"
              : hovered
                ? "0 26px 90px rgba(0,0,0,0.62)"
                : "0 18px 62px rgba(0,0,0,0.48)",
          opacity: selected || hovered ? 1 : 0.88,
        }}
      >
        {node.collapsed ? (
          <div
        ...
        <article
          style={{
            ...
            boxShadow: selected
              ? "0 0 0 1px rgba(255,255,255,0.18), 0 28px 96px rgba(255,255,255,0.2)"
              : dragging
                ? "0 34px 120px rgba(134,239,172,0.16)"
